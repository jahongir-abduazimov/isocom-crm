...
export default function ReprocessingPage() {
    const { recyclings, loading, error, totalCount, fetchRecyclings } = useRecyclingStore();
    ...
    const filteredRecyclings = recyclings.filter((recycling) => {
        const matchesSearch =
            ...
            recycling.scrap_details.reason_display.toLowerCase().includes(searchTerm.toLowerCase()) ||
            recycling.recycled_by_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            recycling.scrap_details.notes.toLowerCase().includes(searchTerm.toLowerCase()) ||
            recycling.notes.toLowerCase().includes(searchTerm.toLowerCase());

        const matchesScrapTypeFilter =
            filterScrapType === "all" || recycling.scrap_details.scrap_type === filterScrapType;
    ...
    const uniqueScrapTypes = [
        ...new Set(recyclings.map((recycling) => recycling.scrap_details.scrap_type)),
    ];
    const uniqueReasons = [...new Set(recyclings.map((recycling) => recycling.scrap_details.reason))];
        ...
        <div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xl lg:text-2xl font-bold text-red-600">
                                    {recyclings.filter((r) => r.scrap_details.scrap_type === "HARD").length}
                                </p>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xl lg:text-2xl font-bold text-orange-600">
                                    {recyclings.filter((r) => r.scrap_details.scrap_type === "SOFT").length}
                                </p>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xl lg:text-2xl font-bold text-green-600">
                                    {recyclings.reduce((sum, r) => sum + parseFloat(r.recycled_quantity), 0).toFixed(2)} KG
                                </p>
                            </div>
                                ...
                                <SelectContent>
                                    ...
                                    {uniqueScrapTypes.map((type) => (
                                        <SelectItem key={type} value={type}>
                                            {recyclings.find((r) => r.scrap_details.scrap_type === type)
                                        ...
                                        <td className="px-3 lg:px-6 py-4 text-sm text-gray-900">
                                            ...
                                            <div className="lg:hidden mt-1">
                                                <span className="text-sm font-medium">
                                                    {recycling.recycled_quantity} {recycling.scrap_details.unit_of_measure}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="hidden lg:table-cell px-3 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-xs text-gray-500">
                                                    Asl: {recycling.scrap_details.quantity} {recycling.scrap_details.unit_of_measure}
                                                </span>
                                            </div>
                                        </td>
            ...
            {loading && (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-lg mt-4">Qayta ishlashlar yuklanmoqda...</p>
                </div>
            )}
